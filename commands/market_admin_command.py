...
def market_admin_command(k):
    ...
    async def market_clear_command(ctx):
        ...
        try:
            ...
            embed.add_field(name="현재 상태", value=f"**등록된 물품:** {item_count}개", inline=False)
            embed.add_field(
                name="⚠️ 경고",
    ...
    async def market_remove_command(ctx, item_code: str = None):
        ...
        try:
            ...
            async with file_lock:
                ...
                embed.add_field(name="물품 이름", value=f"**{target_item.name}**", inline=False)
                embed.add_field(name="물품 코드", value=f"**{item_code}**", inline=True)
                embed.add_field(name="타입", value="🎭 역할" if target_item.is_role else "🎫 티켓", inline=True)
                embed.add_field(name="구매된 티켓/횟수", value=f"**{target_item.tickets_sold}**", inline=True)
                embed.set_footer(text=f"명령어 실행자: {ctx.author.display_name}")
                await ctx.send(embed=embed)
        except Exception as e:
            await ctx.send(f"❌ 오류가 발생했습니다: {e}")
    ...
    async def market_add_ticket_command(ctx, *args):
        ...
        try:
            ...
            async with file_lock:
                new_item = MarketItem(
                    name=item_name, code=item_code, draw_count=draw_count, max_purchase=max_purchase,
                    price_per_ticket=price, quantity=0, tickets_sold=0, buyers=[],
                    is_role=False, role_name=None
                )
                ...
                embed.add_field(name="물품 이름", value=f"**{item_name}**", inline=False)
                embed.add_field(name="물품 코드", value=f"**{item_code}**", inline=True)
                embed.add_field(name="뽑는 인원", value=f"**{draw_count}명**", inline=True)
                embed.add_field(name="1인당 최대 구매", value=f"**{max_purchase}개**", inline=True)
                embed.add_field(name="티켓 가격", value=f"**{price:,} 포인트**", inline=True)
                embed.set_footer(text=f"명령어 실행자: {ctx.author.display_name}")
                await ctx.send(embed=embed)
        except Exception as e:
            await ctx.send(f"❌ 오류가 발생했습니다: {e}")
    ...
    async def market_add_role_command(ctx, *args):
        ...
        try:
            ...
            async with file_lock:
                new_item = MarketItem(
                    name=f"역할: {role_name}", code=item_code, draw_count=1, max_purchase=1,
                    price_per_ticket=price, quantity=0, tickets_sold=0, buyers=[],
                    is_role=True, role_name=role_name
                )
                success = add_market_item("market.txt", new_item)
                if not success:
                    await ctx.send(f"❌ 물품 코드 `{item_code}`가 이미 존재합니다.")
                ...
                embed.add_field(name="역할 이름", value=f"**{role_name}**", inline=False)
                embed.add_field(name="물품 코드", value=f"**{item_code}**", inline=True)
                embed.add_field(name="가격", value=f"**{price:,} 포인트**", inline=True)
                embed.set_footer(text=f"명령어 실행자: {ctx.author.display_name}")
                await ctx.send(embed=embed)
        except Exception as e:
            await ctx.send(f"❌ 오류가 발생했습니다: {e}")
